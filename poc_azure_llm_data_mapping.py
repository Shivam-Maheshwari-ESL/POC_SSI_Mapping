...
from retry import retry_doc_intel_call, retry_llm_call

from openai import AzureOpenAI
from utility import save_txt_to_file, load_llm_credentials, get_model_version, read_json_file_and_convert_to_string, read_json_file_and_convert_to_slim_string, read_csv_file_and_convert_to_string, read_excel_file_and_convert_to_string, slim_doc_intelligence_json, save_json

from prompt_file import get_mapping_prompt

INPUT_JSON_DIR = "./input_ADI_json"
OUTPUT_DIR = "./output_llm_on_ADI"
OUTPUT_DIR_INTERMEDIATE = os.path.join(OUTPUT_DIR, "intermediate_llm_json")
OUTPUT_DIR_FINAL = os.path.join(OUTPUT_DIR, "final_processed_json")
...
def resolve_log_file_path():
    ...
    return os.path.join(LOG_DIR, f"ssi_mapper_batch_{run_stamp}.log")
...
def call_llm_with_retries(prompt, endpoint, api_key, api_version, deployment_name):
    """Send the PDF file itself (no local text/image extraction) + SSI prompt to Azure OpenAI.

    Rate-limit/timeout retries are handled entirely by the `retry_llm_call` (tenacity)
    decorator below, which honors the API's `Retry-After` header on 429s.
    """
    client = AzureOpenAI(
        azure_endpoint=endpoint,
        api_key=api_key,
        api_version=api_version,
        timeout=900,  # large prompts can take several minutes for gpt-5.4 to process
    )
    ...
    try:
        ...
        logger.exception("LLM call failed for deployment '%s'", deployment_name)
        raise
    llm_call_end = time.time()
    llm_response_time_sec = llm_call_end - llm_call_start
    ...
    logger.info(
        "LLM response received from deployment '%s' | "
        "Tokens: prompt=%s, completion=%s, total=%s | Response time: %.2fs",
        deployment_name, prompt_tokens, completion_tokens, total_tokens, llm_response_time_sec
    )
...
def call_llm_with_fallback(prompt, endpoint, api_key, api_version, deployment_candidates):
    ...
    for deployment_name in deployment_candidates:
        ...
        try:
            return call_llm_with_retries(prompt, endpoint, api_key, api_version, deployment_name)
...
def get_prompt(input_json_path):
...
def list_input_json_files(input_dir=INPUT_JSON_DIR):
    """Return sorted list of JSON file names (without extension) found in input_dir."""
    if not os.path.isdir(input_dir):
        raise FileNotFoundError(f"Input JSON directory '{input_dir}' not found.")
...
def process_single_json(json_file_name, endpoint, api_key, api_version, deployment_candidates,
                         example_json, country_code_mapping, mapping_rule_excel_data):
    """Run the LLM mapping pipeline for a single input JSON file and persist outputs."""
    doc_processing_start = time.time()
    json_file_path = os.path.join(INPUT_JSON_DIR, f"{json_file_name}.json")
    model_version = get_model_version()

    output_file_intermediate_llm = os.path.join(
        OUTPUT_DIR_INTERMEDIATE, f"output_{json_file_name}_{model_version}_intermediate.json"
    )
    output_file_final = os.path.join(
        OUTPUT_DIR_FINAL, f"output_{json_file_name}_{model_version}.json"
    )

    logger.info("=" * 80)
    logger.info("Processing input JSON file: %s", json_file_path)

    # Strip DI word/line polygons and bounding regions - they inflate the prompt to
    # hundreds of thousands of tokens without adding mapping value, and were the
    # direct cause of persistent 429 (TPM) errors on the gpt-5.4 deployment.
    data_json = read_json_file_and_convert_to_slim_string(json_file_path)
    logger.info("Loaded input data JSON chars (slimmed): %s", len(data_json))

    prompt = get_mapping_prompt(mapping_rule_excel_data, country_code_mapping, example_json, data_json)

    prompt_save_file_path = os.path.join(OUTPUT_DIR, "temp", f"prompt_{json_file_name}_{model_version}.txt")
    logger.info("Saving prompt to file : %s", prompt_save_file_path)
    save_txt_to_file(prompt, prompt_save_file_path)

    logger.info("Final prompt chars: %s", len(prompt))
    ...
    save_json(result, output_file_final)
    logger.info("Saved final mapped output: %s", output_file_final)

    doc_processing_end = time.time()
    doc_processing_time_sec = doc_processing_end - doc_processing_start
    logger.info(
        "Document processing completed for '%s' | Total time: %.2fs",
        json_file_name, doc_processing_time_sec
    )
    logger.info("Extracted data preview for %s: %s", json_file_name, json.dumps(result, ensure_ascii=False)[:500])
    logger.info("=" * 80)
...
def main():
    ...
    try:
        logger.info("SSI mapping batch run started")
        ...
        logger.info("Using input JSON directory: %s", INPUT_JSON_DIR)
        ...
        json_file_names = list_input_json_files()

        print("json_file_names:", json_file_names)
        json_file_names= json_file_names[0:1] 
        print("json_file_names after slicing:", json_file_names)

        logger.info("Found %s input JSON files to process", len(json_file_names))
        ...
        for json_file_name in json_file_names:
            try:
                process_single_json(
                    ...
                    mapping_rule_excel_data,
                )
            except Exception as e:
                logger.exception("Failed to process '%s': %s", json_file_name, e)
                continue

        logger.info("SSI mapping batch run completed")
    except Exception as e:
